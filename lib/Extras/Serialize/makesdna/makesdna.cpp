...
int add_name(char *str) {
    ...
    if (str[0] == '(' && str[1] == '*') {
        ...
        if (debugSDNA > 3)
            printf("first brace after offset %d\n", i);
        ...
        if (debugSDNA > 3)
            printf("seen %c ( %d) \n", str[j], str[j]);
        if (debugSDNA > 3)
            printf("special after offset %d\n", j);

        if (str[j] == 0) {
            if (debugSDNA > 3)
                printf("offsetting for space\n");
...
int make_structDNA(char *baseDirectory, FILE *file) {
    ...
    if (debugSDNA)
        printf("\tFinished scanning %d headers.\n", i);
